...
class Reader:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(args) > 0:
            if (PYTHON3 and type(args[0]) is type("stringTest")) or (
                    not PYTHON3 and isinstance(args[0], (str, unicode))):
                self.load(args[0])
                return
    ...
    def shapeRecordDicts(self):
        """Returns a list of combination geometry/attribute records for
        all records in a shapefile, with the records in a dictionary"""
        shapeRecords = []
        fieldNames = [_x[0] for _x in self.fields[1:]]
        return [_ShapeRecord(shape=rec[0], record=dict(zip(fieldNames, rec[1]))) \
                                for rec in zip(self.shapes(), self.records())]
    
    def shapeRecordDataFrame(self):
        """Returns a list of combination geometry/attribute records for
